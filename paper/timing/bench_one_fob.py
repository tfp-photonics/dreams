...
def build_problem(num, lmax, lmax_glob):
    ...
    tetalist = jnp.ones((Nteta, Nphi)) * jnp.linspace(tetamin, tetamax, Nteta)[:, None]
    philist = (
        jnp.ones((Nteta, Nphi))
        * jnp.linspace(phimin, phimax, Nphi, endpoint=False)[None, :]
    )

    xff = (r * jnp.sin(tetalist) * jnp.cos(philist)).flatten()
    yff = (r * jnp.sin(tetalist) * jnp.sin(philist)).flatten()
    zff = (r * jnp.cos(tetalist)).flatten()

    d_solid_surf = (r**2 * jnp.sin(tetalist) * dteta * dphi).flatten()

    grid_f = jnp.transpose(jnp.array([xff, yff, zff]))
    grid_b = -grid_f
...
def main():
    ...
    ap.add_argument("--jit_forward", type=int, default=1)
    ap.add_argument("--jit_vg", type=int, default=1)
    args = ap.parse_args()

    cfg, param = build_problem(args.num, args.lmax, args.lmax_glob)

    f = lambda p: fob(p, cfg)
    vg = lambda p: value_and_grad(fob)(p, cfg)
    ...
    print("calling f(param) first time", flush=True)
    block(f(param))
    print("calling vg(param) first time", flush=True)
    block(vg(param))
